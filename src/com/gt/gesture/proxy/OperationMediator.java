...
public class OperationMediator {
    ...
    private GestureFeature[] getFeature(RawFeature rf) {
        GestureFeatureExtractor featureExtractor = new GestureFeatureExtractor(rf);
    ...
    public boolean generateCodeBook() {
        ...
        for (Model[] regModel : regModels) {
            for (int j = 0; j < regModel.length; j++) {
                GestureFeature[] gf = getFeature((RawFeature) regModel[j]);
                for (GestureFeature aGf : gf) {
                    allFeaturesList.add(aGf);
                    totalFrames++;
                }
            }
